document.addEventListener("DOMContentLoaded", () => {
    ...
    function autenticarUsuario() {
        const usuario = prompt("Digite seu nome de usuário:")?.trim();
        const senha = prompt("Digite sua senha:")?.trim();
        if (usuario === "corretor" && senha === "1234") {
            isCorretor = true;
            alert("Bem-vindo, corretor!");
        } else {
            alert("Acesso negado. Apenas corretores podem gerenciar imóveis.");
        }
    }
    ...
    function renderizarImoveis(lista = imoveis) {
        ...
        lista.forEach((imovel, index) => {
            ...
            imovelCard.classList.add("imovel-card");
    ...
    // Autenticar usuário ao carregar a página
    autenticarUsuario();
